...
from app.models import CartItems, db, User, PurchasedCourses
...
def add_item_to_cart():
    try:
        ...
        check = PurchasedCourses.query.filter_by(user_id=user_id, product_id=product_id).first()

        if check:
            return send_result(message='Bạn đã mua khóa học này')

        check_item = CartItems.query.filter_by(product_id=product_id, user_id=user_id).first()
        if check_item:
            check_item.modified_date = get_timestamp_now()
        else:
            item = CartItems(id=str(uuid()), product_id=product_id, user_id=user_id)
            db.session.add(item)
        db.session.flush()
        db.session.commit()
